...
class CiudadControlador extends Ciudad{

    public function __construct(){}
    public function verificacioninsertar($nombre,$Paises_id,$descripcion){
    ...
    public function verificacionmostrar(){
        ...
        $pais = new Pais();
        $objetoretornadopais = $this->mostrarpaises();
        $pais = array_unique($objetoretornadopais,SORT_REGULAR);
        require '../vistas/ciudades/mostrar.php';
    }
    public function verimostpaisyhosp(){
        // echo "Estamos llegando a la funcion Mostrar";
        $objetoretornado = $this->mostrarpaises();
        require '../vistas/ciudades/formulario.php';
    }
    public function verificacionactualizar($id){
        $this->id = $id;
        $objetoretornado = $this->actualizar();
       
        require '../vistas/ciudades/actualizar.php';
    }
    ...
    public function redireccionar(){
        header("location: CiudadControlador.php?accion=mostrar");
    }
  ...
  if (isset($_SESSION['email']) && ($_SESSION['rol'] == "Administrador")){  
       if (isset($_GET['accion'])&&$_GET['accion']=="insertar") {
        $instanciacontrolador = new CiudadControlador();
        $instanciacontrolador->verimostpaisyhosp();
    }
    if (isset($_POST['accion']) && $_POST['accion']=="insertar") { //El isset es para verificar si la variable esta llegando la informacion vacia o no
        if(isset($_POST['nombre'])&&isset($_POST['pais'])&&isset($_POST['descripcion'])){
            $instanciacontrolador = new CiudadControlador();
            $instanciacontrolador->verificacioninsertar($_POST['nombre'],$_POST['pais'],$_POST['descripcion']);  
        }
    else {
